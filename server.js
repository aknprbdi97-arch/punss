...
// ========== MAINTENANCE MIDDLEWARE (VERSI ALTERNATIF) ==========
app.use((req, res, next) => {
  // Daftar path yang DIJAGA (tidak kena maintenance)
  const allowedPaths = ['/maintenance.html', '/api/maintenance', '/api/check-maintenance'];
  ...
  if (maintenanceMode === true) {
    // Kirim file maintenance.html dengan status 503
...
app.post('/api/create', async (req, res) => {
  ...
  const pterodactylUsername = panelUsername;
...
// ========== SERVE FRONTEND DENGAN CEK MAINTENANCE ==========
app.get('*', (req, res) => {
  // Cek maintenance mode untuk semua route non-API
  if (maintenanceMode === true && req.path !== '/maintenance.html' && !req.path.startsWith('/api/')) {
